...
华为毕昇编译器（Bisheng Compiler）基于LLVM开发，并针对鲲鹏架构进行了优化。以下是其主要的语法规则限制：

1. **Clang 不支持的选项和功能**【5†source】：
   - **print-multi-os-directory**: Clang不支持`print-multi-os-directory`选项，编译时需要通过硬编码来指定路径。
   - **-fstack-clash-protection**: 该选项尚未被支持。
   - **内置函数限制**: Clang在AArch64后端不支持`__builtin_longjmp`和`__builtin_setjmp`等内置函数。
   - **__uint128_t**: Clang在32位模式下不支持`__uint128_t`类型。
   - **NEON 指令限制**: Clang不支持NEON指令中`Q`寄存器的使用。
   - **运行库支持不足**: Clang默认不支持一些libgcc中的符号，建议使用`--rtlib=compiler-rt`以支持这些符号。
   - **原子类型转换**: 不支持对原子类型（atomic type）的类型转换。

2. **链接选项限制**【5†source】：
   - 生成动态库和PIE可执行文件时需要手动指定`-fPIC`和`-pie`，Clang严格区分编译和链接选项。
   - 传递给链接器的参数必须加`-Wl`，如`-znow`、`--whole-archive`等。

3. **Fortran 兼容性**【5†source】：
   - **main 函数双重定义**: Fortran与C混合编程时，需要在链接时加上`-fno-fortran-main`，否则可能会发生`main`函数重定义。
   - **多维数组大小限制**: 对于Flang编译的多维数组，当生成的中间表示（IR）文件大于2GB时，编译可能失败。

4. **浮点运算控制**【5†source】：
   - 浮点精度控制支持多种选项，如`-ffp-contract=style`和`-faarch64-pow-alt-precision`，以确保在非ARM平台的计算一致性。
   - 其他控制选项包括`-mllvm -disable-sincos-opt`等，用于确保跨平台的计算一致性。

5. **自定义优化选项**【5†source】：
   - 毕昇编译器支持通过`-mllvm`驱动的一些自定义优化选项，包括强制使用自定义pass顺序、增强LLVM的值域分析、和使能函数参数优化。
   - 支持多种矢量化和循环优化控制指令，如`-enable-loop-split`、`-enable-sroa-after-unroll`等，提升代码运行效率。

这些规则和限制帮助开发者在AArch64架构上优化性能，但在移植代码时需注意这些不兼容特性，以确保程序的正确性和高效执行。
