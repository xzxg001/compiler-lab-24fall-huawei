...
class Scope {
public:
    // enter a new scope// 进入一个新的作用域
    void enter() {
        inner.push_back({});
    }

    // exit a scope// 退出一个作用域
    void exit() {
        inner.pop_back();
    }

    bool in_global() {// 判断当前是否在全局作用域内
        return inner.size() == 1;
    }
    ...
    bool push(std::string name, Value *val) {// 往当前作用域插入新的名字->值映射
        auto result = inner[inner.size() - 1].insert({name, val});
        return result.second;
    }

    Value* find(std::string name) {// 根据名字，寻找到值
